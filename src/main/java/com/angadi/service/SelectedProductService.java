...
public class SelectedProductService {
	...
	public ResponseEntity<ResponseStructure<SelectedProduct>> addSelectedProduct(ProductDto product, long customerId, int productQuantity) {
		SelectedProduct selectedProduct = new SelectedProduct();
		selectedProduct.setProductName(product.getProductName());
		selectedProduct.setProductDescription(product.getProductDescription());
		selectedProduct.setProductPrice(product.getProductPrice());
		selectedProduct.setProductQuantity(productQuantity);
		selectedProduct.setTotalProductPrice(productQuantity*selectedProduct.getProductPrice());
	...
	public ResponseEntity<ResponseStructure<SelectedProduct>> updateSelectedProduct(int selectedProductQuantity, long selectedProductId) {
		SelectedProduct selectedProduct2 = selectedProductDao.getselectedProduct(selectedProductId);
		if(selectedProduct2!=null) {
			selectedProduct2.setProductQuantity(selectedProductQuantity);
			selectedProduct2.setTotalProductPrice(selectedProductQuantity*selectedProduct2.getProductPrice());
